...
Quick-reference for the libraries Alan reaches for by purpose. Inferred from a sample of repos under `~/dev/` — bias toward modern projects (Yappr, RecallOS, Printr, Printr MCP, Bloomate, SafeURL, Memeprintr, Pousadamariabonita, Printr-presale). Legacy / experiments (re-reduced, lovable templates, single-RN apps) are excluded.
...
- **Acceptable**: pnpm in work monorepos that already use it (Printr web, Sonar).
...
- **Acceptable**: **Elysia** where the repo already uses it (SafeURL dashboard, gpt-proxy, Dreamquest, Bloomate API).
- **Avoid**: Express, Koa, Fastify (Fastify only appears in Greenbot/Svelte).
...
- **Default**: **`neverthrow`** (`Result` / `ResultAsync`) at service, IO, business, API, DB, and tool-handler boundaries.
- **Lint**: `eslint-plugin-neverthrow` when ESLint is in use and the codebase wants enforcement (mymcp).
- **Maybe**: RecallOS-style `Maybe<T> = Result<T, None>` for expected absence — copy that shape only when the repo already has it.
- **Avoid**: fp-ts, effect, purify-ts, true-myth. Do not introduce a second result library alongside neverthrow.
...
- **Default**: **`pino` + `pino-pretty`** for service code (RecallOS family).
...
- **Default**: **React 19** for web and webviews; **Ink + React** for terminal UIs (Yappr CLI, Memeprintr).
- **Acceptable**: Svelte / SvelteKit, Lit, Lynx, Ripple — only as intentional experiments. Do not propose them for a new general-purpose project.
- **Avoid**: Vue, Solid, Preact in app code (styled-cva publishes adapters but apps do not consume them).

## Router

- **Default for new SPAs / Vite apps**: **TanStack Router** (Memeprintr, Pousadamariabonita, Printr-presale, Bloomate web).
- **Default for SSR full-stack on Cloudflare**: **TanStack Start** (Pousadamariabonita, Printr-presale).
- **Acceptable**: **React Router v7** when the repo already uses it (Printr web); **Next.js** App Router for content-first sites (alanrsoares.me, Cannaspy, Wharemoni, Weirdle).
...
- **Default for new web apps**: **`@tanstack/react-store`** (Pousadamariabonita, Printr-presale).
- **Acceptable**: **`zustand`** in repos that already use it (Printr web, Weirdle); **`unstated-next`** for small per-screen stores (older repos).
- **Avoid**: Redux, MobX, Jotai, Valtio — Redux only survives in re-reduced (legacy).

## HTTP Client

- **Default**: native `fetch` (Bun + Workers + browser).
- **Acceptable**: **`ky`** when a thin wrapper is wanted (Gpt-proxy, Magnetomatic, Greenbot).
...
- **Acceptable**: **`tsup`** for publishable TS libraries (Sonar, styled-cva, Studio).
...
- **Acceptable**: `daisyui` for quick prototypes; `@pandacss/dev` only where it already lives (Studio).
...
- **Acceptable**: **`@base-ui/react`** primitives where the project moved to Base UI (Bloomate, Printr web).
- **Avoid**: MUI, Chakra, Ant Design, Mantine, NextUI.

## Forms

- **Default for new code**: **`@tanstack/react-form`**.
- **Acceptable**: **`react-hook-form` + `@hookform/resolvers/zod`** in older lovable-template repos — keep if already there.
- **Avoid**: Formik.
...
- **End-to-end**: **Playwright** when the repo already has it (Bliv, Printr web, Studio).
...
- **Desktop default**: **Electrobun** + typed RPC to a Bun-side data/service layer (Yappr).
- **Acceptable**: **Tauri** only for non-Bun host integrations (tauri-open-webui).
- **Mobile**: **Expo / React Native** only when targeting mobile is the stated goal (NasToolsApp).
- **Avoid**: Electron.

## AI SDKs

- **Agent framework**: **Mastra** (`@mastra/*`) when building agentic systems (Memeprintr).
- **Chat / streaming UIs**: **Vercel AI SDK** (`ai`, `@ai-sdk/react`) (Yappr desktop, Bloomate).
- **Local inference**: **`ollama`** + the appropriate `ollama-ai-provider-v2` adapter (RecallOS, Yappr).
- **Model routing**: **`@openrouter/sdk`** (RecallOS); **`workers-ai-provider`** on Cloudflare (Bloomate).
...
- **Pattern**: terminate `ResultAsync` pipelines at the MCP handler with a repo-local `toToolResponseAsync()` helper (Printr MCP).

## Codegen And SDKs

- **OpenAPI → TS types**: **`openapi-typescript`** (Mymcp, Printr MCP, Yappr).
- **OpenAPI → client**: **`@hey-api/openapi-ts`** (safeurl-sdk).
...
- **CLI prompts/parsing**: **`commander` + `inquirer`** for CLI tools (nas-tools).
...
- **Default for full-stack**: **Cloudflare Workers** (Wrangler) with **TanStack Start** or **Hono** (Pousadamariabonita, Bloomate, Printr-presale, Printr CDN).
